...
const generated=[];
try{
  console.log("Kenji Pass 21: new-user onboarding → owner signup control → login recovery + auth hardening");
  ...
  console.log("Applying Pass 21 D1 onboarding migration…");
  npx(["d1","migrations","apply",DB_NAME,"--remote","--config",dataConfig]);
  console.log("Deploying Kenji Overwatch Pass 21…");
  ...
  console.log("Onboarding: New User Sign Up is available from the login tile while owner registration remains enabled.");
  console.log("Owner Setup: Access Control can disable future signup without disabling existing logins.");
  console.log("Auth: first account becomes Owner; later accounts become Operator; existing 180k PBKDF2 hashes remain compatible and new accounts use 240k iterations.");
  console.log("Hardening: signup/login throttling, same-origin auth mutations, bounded sessions, generic login errors and owner-only signup controls.");
  console.log("No provider routes, queue topology or shared repositories are modified.");
