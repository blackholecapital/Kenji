...
const generated=[];
try{
  console.log("Kenji Pass 4: inbound receptionist → Twilio routing control → live demo readiness");
  const d1Id=findD1();console.log(`Using D1 ${DB_NAME} (${d1Id})`);
  const dataConfig=generatedConfig(resolve(ROOT,"apps/data-worker/wrangler.toml"),d1Id),voiceConfig=generatedConfig(resolve(ROOT,"apps/voice-worker/wrangler.toml"),d1Id),overwatchConfig=generatedConfig(resolve(ROOT,"apps/overwatch-worker/wrangler.toml"),d1Id);generated.push(dataConfig,voiceConfig,overwatchConfig);
  console.log("Applying Pass 4 D1 migration…");npx(["d1","migrations","apply",DB_NAME,"--remote","--config",dataConfig]);
  console.log("Deploying Kenji Voice Pass 4…");deployWithStore(voiceConfig,{TWILIO_ACCOUNT_SID:"XYZ_DEMO_TWILIO_ACCOUNT_SID",TWILIO_AUTH_TOKEN:"XYZ_DEMO_TWILIO_AUTH_TOKEN",DEEPGRAM_API_KEY:"XYZ_DEMO_DEEPGRAM_API_KEY",EILA_RUNTIME_TOKEN:"XYZ_DEMO_EILA_RUNTIME_TOKEN",INTERNAL_CALL_SECRET:"XYZ_DEMO_EILA_RUNTIME_TOKEN"});
  console.log("Deploying Kenji Overwatch Pass 4…");deployWithStore(overwatchConfig,{INTERNAL_CALL_SECRET:"XYZ_DEMO_EILA_RUNTIME_TOKEN",EILA_RUNTIME_TOKEN:"XYZ_DEMO_EILA_RUNTIME_TOKEN",BLACKHOLE_CAPABILITY_TOKEN:"XYZ_DEMO_EILA_RUNTIME_TOKEN"});
  console.log("\nPass 4 deployed.");
  console.log("Command Center: https://kenji-overwatch-worker.cryptocapitalgroupfl.workers.dev");
  console.log("Inbound webhook: https://kenji-voice-worker.cryptocapitalgroupfl.workers.dev/twilio/inbound");
  console.log("New surface: Live Demo");
  console.log("Important: Pass 4 does not reroute a Twilio number during deployment. Choose a number in Live Demo and explicitly confirm the route change.");
