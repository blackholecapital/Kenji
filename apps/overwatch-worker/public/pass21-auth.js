(() => {
  ...
  function applyAccess(){const btn=$('#p21SignupOpen'),note=$('#p21SignupNote');if(!btn||!note)return;const enabled=access?.signupEnabled!==false;btn.style.display=enabled?'':'none';note.textContent=enabled?'New account registration is temporarily open for approved testers.':'New user registration is currently disabled by the owner.';note.classList.toggle('closed',!enabled);if(!enabled&&signupOpen)setSignup(false);}
  async function loadAccess(){try{access=await api('/api/auth/access');applyAccess();renderOwnerAccess();}catch(e){console.warn('Pass21 access status failed',e);}}
  ...
  function loginSubmitPolish(){const submit=$('#authSubmit');if(submit&&submit.textContent!=='Enter AI Call Center')submit.textContent='Enter AI Call Center';const old=$('#authError');if(old&&!old.dataset.p21){old.dataset.p21='1';const observer=new MutationObserver(()=>{if(/500|request failed/i.test(old.textContent||''))old.textContent='Login could not be completed. Refresh once and try again; if it continues, use New User Sign Up or check Owner Setup access.';});observer.observe(old,{childList:true,characterData:true,subtree:true});}}
  ...
  function boot(){authUi();ensureOwnerAccess();loginSubmitPolish();loadAccess();
    ...
    const auth=$('#authShell');if(auth)new MutationObserver(()=>requestAnimationFrame(()=>{authUi();loginSubmitPolish();})).observe(auth,{childList:true,subtree:true});
  }
